...
class Solution11(unittest.TestCase):
    def setUp(self):
        file_path = os.path.join(os.path.dirname(__file__), '..', 'problem_data', 'p011_grid.txt')
        with open(file_path) as f:
            grid = [[int(x) for x in line.split()] for line in f.readlines()]
        self.problem = Problem11(grid)

    def test_solution(self):
        self.assertEqual(70600674, self.problem.solve())
